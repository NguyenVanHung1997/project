...
void Carofile::readPlayerRankFromFile()
{
    string buffer;
    ...
    while (!ifListPlayer.eof())
    {
        ...
        newPlayer->setLose(stoi(lose));
        newPlayer->setDraw(stoi(draw));
...
void Carofile::writePlayerRankToFile()
{
...
void Carofile::setNamePlayer()
{
    ...
    cin >> name2;
    if (mPlayers[name1] != nullptr && mPlayers[name1]->getName().compare(name1) == 0)
    {
    ...
    if (mPlayers[name2] != nullptr && mPlayers[name2]->getName().compare(name2) == 0)
    {
...
void Carofile::writeLastPlayerToFile()
{
    ofListPlayer.open("lastPlayer.txt", ios::out);

    ofListPlayer << player1->getName() << "\n";
    ofListPlayer << player2->getName() << "\n";

    ofListPlayer.close();
}

void Carofile::readLastPlayerFromFile()
{
    ifListPlayer.open("lastPlayer.txt");
    if(!ifListPlayer.is_open()) {
        return;
    }
    string buff;
    getline(ifListPlayer, buff);
    player1 = mPlayers[buff];
    getline(ifListPlayer, buff);
    player2 = mPlayers[buff];
    ifListPlayer.close();
}
